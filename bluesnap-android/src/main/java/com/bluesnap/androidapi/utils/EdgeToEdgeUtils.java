...
public class EdgeToEdgeUtils {
    ...
    public static void enableEdgeToEdge(Activity activity) {
        if (Build.VERSION.SDK_INT >= 29) { // Android 10+ (API 29+) - covers Android 15 and other versions with gesture navigation
    ...
    /**
     * Apply bottom system bar insets only (for button areas that need to avoid navigation bars)
     * This ensures buttons remain clickable with both gesture and button navigation
     */
    public static void applyBottomSystemBarInsets(View view) {
        if (Build.VERSION.SDK_INT >= 29) { // Android 10+ (API 29+)
            ViewCompat.setOnApplyWindowInsetsListener(view, (v, windowInsets) -> {
                Insets insets = windowInsets.getInsets(WindowInsetsCompat.Type.systemBars());
                
                // Only apply bottom inset to avoid navigation bar overlap
                ViewGroup.MarginLayoutParams params = (ViewGroup.MarginLayoutParams) v.getLayoutParams();
                if (params != null) {
                    params.bottomMargin = insets.bottom;
                    v.setLayoutParams(params);
