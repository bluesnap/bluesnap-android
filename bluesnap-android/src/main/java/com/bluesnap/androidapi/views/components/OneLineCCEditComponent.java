...
public class OneLineCCEditComponent extends LinearLayout {
    ...
    private void initControl(Context context) {
        ...
        cvvEditText.setOnFocusChangeListener(new View.OnFocusChangeListener() {
            ...
            public void onFocusChange(View v, boolean hasFocus) {
                if (!hasFocus) {
                    cvvValidation();
                     BlueSnapLocalBroadcastManager.INSTANCE.sendMessage(getContext(), BlueSnapLocalBroadcastManager.ONE_LINE_CC_EDIT_FINISH, TAG);
                } else
    ...
    private boolean cardNumberValidation(String creditCardNumber) {
        boolean validationResult = BlueSnapValidator.Companion.getInstance().creditCardNumberValidation(AndroidUtil.stringify(creditCardNumber));
    ...
    private boolean cvvValidation() {
        ...
        boolean validationResult = BlueSnapValidator.Companion.getInstance().creditCardCVVValidation(cvv, newCreditCard.getCardType());
        if (validationResult)
    ...
    private boolean expValidation() {
        ...
        boolean validationResult = BlueSnapValidator.Companion.getInstance().creditCardExpiryDateValidation(exp);
        if (validationResult)
    ...
    private void submitCCNumber() {
        final BlueSnapService blueSnapService = BlueSnapService.Companion.getInstance();
