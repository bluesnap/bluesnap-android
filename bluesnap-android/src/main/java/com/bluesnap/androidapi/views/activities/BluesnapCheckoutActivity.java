...
public class BluesnapCheckoutActivity extends AppCompatActivity {
    ...
    protected final BlueSnapService blueSnapService = BlueSnapService.Companion.getInstance();
    ...
    private void checkIsGooglePayAvailable() {
        ...
        GooglePayService googlePayService = GooglePayService.Companion.getInstance();
    ...
    protected void startPayPalActivityForResult() {
        String payPalToken = BlueSnapService.Companion.getPayPalToken();
    ...
    protected void startGooglePayActivityForResult() {
        ...
        Task<PaymentData> futurePaymentData = GooglePayService.Companion.getInstance().createPaymentDataRequest(googlePayClient);
    ...
    protected void startPayPal(final PriceDetails priceDetails) {
        ...
        BlueSnapService.Companion.getInstance().createPayPalToken(priceDetails.getAmount(), priceDetails.getCurrencyCode(), new BluesnapServiceCallback() {
            @Override
            public void onSuccess() {
                try {
                    startWebViewActivity(BlueSnapService.Companion.getPayPalToken());
                } catch (Exception e) {
            ...
            public void onFailure() {
                try {
                    JSONObject errorDescription = BlueSnapService.Companion.getErrorDescription();
    ...
    private void handleGooglePaySuccess(PaymentData paymentData) {
        ...
        SdkResult sdkResult = GooglePayService.Companion.getInstance().createSDKResult(paymentData);
        ...
        if (encodedToken == null) {
            ...
            blueSnapService.getAppExecutors().networkIO().execute(new Runnable() {
                ...
                public void run() {
                    // post the token
                    try {
                        BlueSnapHTTPResponse response = BlueSnapService.Companion.getInstance().submitTokenenizedPayment(encodedToken, SupportedPaymentMethods.GOOGLE_PAY);
