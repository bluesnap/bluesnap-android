...
public class AmountTaxShippingComponent extends LinearLayout {
    ...
    private TextView subscriptionCancellationTextView;
    ...
    private void initControl(final Context context) {
        ...
        subscriptionCancellationTextView = findViewById(R.id.subscriptionCancellationTextView);
//        storeCardTextView = findViewById(R.id.storeCardTextView);
        ...
        shippingSameAsBillingSwitch.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
            ...
            public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
                isShippingSameAsBilling = isChecked;
                 BlueSnapLocalBroadcastManager.INSTANCE.sendMessage(getContext(), BlueSnapLocalBroadcastManager.SHIPPING_SWITCH_ACTIVATED, isChecked, TAG);
            }
        });
    ...
    public void setAmountTaxShipping() {
        ...
        Shopper shopper = BlueSnapService.Companion.getInstance().getsDKConfiguration().getShopper();
        ...
        if (sdkRequest.isHideStoreCardSwitch()) {
            ...
            subscriptionCancellationTextView.setVisibility(GONE);
        } else {
            storeCardRelativeLayout.setVisibility(VISIBLE);
            // Show cancellation message only for subscription charges and if enabled
            if (sdkRequest instanceof SdkRequestSubscriptionCharge && sdkRequest.isShowSubscriptionCancellationMessage()) {
                subscriptionCancellationTextView.setVisibility(VISIBLE);

                // Use custom message if provided, otherwise use default string resource
                String customMessage = sdkRequest.getCustomSubscriptionCancellationMessage();
                if (customMessage != null && !customMessage.trim().isEmpty()) {
                    subscriptionCancellationTextView.setText(customMessage);
                } else {
                    subscriptionCancellationTextView.setText(R.string.can_cancel_subscriptions);
                }
            } else {
                subscriptionCancellationTextView.setVisibility(GONE);
            }
    ...
    public void setSubscriptionCancellationVisibility(int visibility) {
        if (subscriptionCancellationTextView != null) {
            this.subscriptionCancellationTextView.setVisibility(visibility);
        }
    }

    /**
     * Updates the subscription cancellation message text and visibility based on current configuration
     */
    public void updateSubscriptionCancellationMessage() {
        if (subscriptionCancellationTextView != null && sdkRequest != null) {
            if (sdkRequest instanceof SdkRequestSubscriptionCharge && sdkRequest.isShowSubscriptionCancellationMessage()) {
                subscriptionCancellationTextView.setVisibility(VISIBLE);
    ...
    public boolean validateStoreCard(boolean isShopperRequirements, boolean isSubscriptionCharge) {
        if (!BlueSnapValidator.Companion.getInstance().validateStoreCard(isShopperRequirements, isSubscriptionCharge, isStoreCard)) {
