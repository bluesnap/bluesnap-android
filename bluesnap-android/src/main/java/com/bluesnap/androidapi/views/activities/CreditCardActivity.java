...
import com.bluesnap.androidapi.utils.EdgeToEdgeUtils;
...
public class CreditCardActivity extends AppCompatActivity {
    ...
    private final BlueSnapService blueSnapService = BlueSnapService.Companion.getInstance();
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        if (savedInstanceState != null && BlueSnapService.Companion.getInstance().getSdkRequest() == null) {
        ...
        // Apply edge-to-edge insets to header to avoid status bar overlap
        View headerLayout = findViewById(R.id.headerRelativeLayout);
        if (headerLayout != null) {
            EdgeToEdgeUtils.applyTopSystemBarInsets(headerLayout);
        }
        ...
        if (BlueSnapService.Companion.getInstance().getSdkRequest().isAllowCurrencyChange()) {
         ...
         BlueSnapLocalBroadcastManager.INSTANCE.registerReceiver(this, BlueSnapLocalBroadcastManager.COUNTRY_CHANGE_REQUEST, broadcastReceiver);
         BlueSnapLocalBroadcastManager.INSTANCE.registerReceiver(this, BlueSnapLocalBroadcastManager.STATE_CHANGE_REQUEST, broadcastReceiver);
    ...
    protected void onDestroy() {
        super.onDestroy();
         BlueSnapLocalBroadcastManager.INSTANCE.unregisterReceiver(this, broadcastReceiver);
    }
    ...
    private void startActivityWithNewCreditCardFragment() {
         BlueSnapLocalBroadcastManager.INSTANCE.registerReceiver(this, BlueSnapLocalBroadcastManager.NEW_CARD_SHIPPING_CHANGE, broadcastReceiver);
    ...
    private void startActivityWithReturningShopperCreditCardFragment() {
         BlueSnapLocalBroadcastManager.INSTANCE.registerReceiver(this, BlueSnapLocalBroadcastManager.SUMMARIZED_BILLING_CHANGE, broadcastReceiver);
         BlueSnapLocalBroadcastManager.INSTANCE.registerReceiver(this, BlueSnapLocalBroadcastManager.SUMMARIZED_BILLING_EDIT, broadcastReceiver);
         BlueSnapLocalBroadcastManager.INSTANCE.registerReceiver(this, BlueSnapLocalBroadcastManager.SUMMARIZED_SHIPPING_CHANGE, broadcastReceiver);
         BlueSnapLocalBroadcastManager.INSTANCE.registerReceiver(this, BlueSnapLocalBroadcastManager.SUMMARIZED_SHIPPING_EDIT, broadcastReceiver);
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (resultCode == Activity.RESULT_OK) {
            if (requestCode == RESULT_COUNTRY)
                 BlueSnapLocalBroadcastManager.INSTANCE.sendMessage(getApplicationContext(), BlueSnapLocalBroadcastManager.COUNTRY_CHANGE_RESPONSE, data.getStringExtra("result"), TAG);
            else if (requestCode == RESULT_STATE)
                 BlueSnapLocalBroadcastManager.INSTANCE.sendMessage(getApplicationContext(), BlueSnapLocalBroadcastManager.STATE_CHANGE_RESPONSE, data.getStringExtra("result"), TAG);
        }
    }
    ...
    private class hamburgerMenuListener implements View.OnClickListener {
        ...
        public void onClick(final View v) {
            sharedCurrency = BlueSnapService.Companion.getInstance().getSdkRequest().getPriceDetails().getCurrencyCode();
    ...
    public void finishFromFragment(final Shopper shopper) {
        ...
        sdkRequest = BlueSnapService.Companion.getInstance().getSdkRequest();
    ...
    private void cardinal3DS(PurchaseDetails purchaseDetails, Shopper shopper, final Intent resultIntent, BlueSnapHTTPResponse response) {
        ...
        BroadcastReceiver broadcastReceiver = new BroadcastReceiver() {
         ...
         BlueSnapLocalBroadcastManager.INSTANCE.registerReceiver(this, CardinalManager.THREE_DS_AUTH_DONE_EVENT, broadcastReceiver);
    ...
    private void finishFromActivity(Shopper shopper, final Intent resultIntent, BlueSnapHTTPResponse response) {
        try {
            String Last4;
            String ccType;
            SdkResult sdkResult = BlueSnapService.Companion.getInstance().getSdkResult();

            if (shopper.getNewCreditCardInfo().getCreditCard().isNewCreditCard()) {
                // New Card
            ...
            sdkResult.setToken(BlueSnapService.Companion.getInstance().getBlueSnapToken().getMerchantToken());
