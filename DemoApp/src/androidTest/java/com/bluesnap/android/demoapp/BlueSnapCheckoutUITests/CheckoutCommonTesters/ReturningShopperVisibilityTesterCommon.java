...
//        String cardLastDigit = TestUtils.getText(isDescendantOfA(withId(R.id.oneLineCCViewComponentsListView)));
    ...
    public static void summarized_contact_info_visibility_validation(String testName, int componentResourceId, boolean fullInfo, boolean withEmail, TestingShopperContactInfo contactInfo) {
        ...
        // Apply retry logic to name field validation
        EspressoSyncUtils.retryOperation(() -> {
            onView(allOf(withId(R.id.nameTextView), isDescendantOfA(withId(componentResourceId))))
                    .perform(EspressoSyncUtils.waitForViewToBeDisplayedWithRetry())
        ...
        }, 2, "name TextView validation");

        if (withEmail) {
            // Wait for UI to be properly updated and then validate email content with retry
            EspressoSyncUtils.retryOperation(() -> {
        ...
        if (fullInfo) {
            ...
            // Apply retry logic to city field validation
            EspressoSyncUtils.retryOperation(() -> {
            ...
            }, 2, "city TextView validation");

            // Apply retry logic to address field validation
            EspressoSyncUtils.retryOperation(() -> {
