...
//            changeCountry(componentResourceId, country);
        // Improved name input with proper synchronization
        EspressoSyncUtils.safeTypeTextWithParent(R.id.input_name, componentResourceId, contactInfo.getName());
        onView(allOf(withId(R.id.input_name), isDescendantOfA(withId(componentResourceId)))).perform(pressImeActionButton());

        if (withEmail) {
            // Improved email input with proper synchronization
            EspressoSyncUtils.safeTypeTextById(R.id.input_email, contactInfo.getEmail());
            onView(withId(R.id.input_email)).perform(pressImeActionButton());
        }

        if (TestUtils.checkCountryHasZip(country)) {
            // Improved zip input with proper synchronization
            EspressoSyncUtils.safeTypeTextWithParent(R.id.input_zip, componentResourceId, contactInfo.getZip());
            onView(allOf(withId(R.id.input_zip), isDescendantOfA(withId(componentResourceId)))).perform(pressImeActionButton());
        }
